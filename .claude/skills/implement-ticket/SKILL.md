...
- **`project-manager`** — slices plan phases or user ideas into tickets appended to `BACKLOG.md`; creates feature folders under `docs/`. Cannot write code or run code. *Currently not used in this skill — there are enough tickets; PM will move into a future `/research` skill.*
- **`team-lead`** — plans and verdicts. Mode 1A (start of ticket): decides whether to dispatch the architect first or skip straight to a developer brief. Mode 1B (post-architect): composes the developer brief from `plans/CCR-NNN-<slug>.md`. **Mode 1C (post-developer, pre-reviewer): refreshes `docs/<feature>/BRIEF.md` and `CONTEXT.md` from the developer's `## BRIEF update note (CCR-NNN)` section, then dispatches the reviewer.** Mode 2 (final): reads the reviewer's report and either ticks acceptance + flips `[done]` + moves the ticket BACKLOG.md → DONE.md and returns APPROVED, or returns a fix dispatch. Cannot write or run code.
- **`architect`** — design pass: reads the codebase + ticket, decides patterns / file placement / abstractions, writes a plan file at `plans/CCR-NNN-<slug>.md`. May include short illustrative code sketches but writes no production code or tests. Runs only when team-lead Mode 1A asks.
...
The shared protocol — ticket schema, status transitions, BRIEF/CONTEXT format, handoff verdict strings, the per-ticket flow — lives in `docs/WORKFLOW.md`. Read it before invoking any agent.
...
- `docs/<feature>/BRIEF.md` — short feature summary, written by team lead on feature completion.
- `docs/<feature>/CONTEXT.md` — file structure + change history per feature, written by team lead from the developer's per-ticket report.
- `plans/CCR-NNN-<slug>.md` — per-ticket design plan, written by the architect when team-lead Mode 1A asks. The developer reads it and follows it; the reviewer reads it and flags undocumented deviations.
...
1. **Read state.** `docs/WORKFLOW.md` (protocol), `BACKLOG.md` (active queue), `DONE.md` only when looking up history of a finished ticket, the relevant phase section of `claude-code-remote-plan.md` (context).
...
7. **(Architect path only) Dispatch `architect`.** Pass the `## Architect brief (CCR-NNN)` section verbatim and tell the architect the slug so it writes to the right path. Architect reads the codebase, writes `plans/CCR-NNN-<slug>.md`, and returns `PLAN READY: CCR-NNN` with a short executive summary. If it returns `BLOCKED: CCR-NNN — <reason>`, surface the reason to the user and stop — do not proceed.
8. **(Architect path only) Dispatch `team-lead` (Mode 1B — compose dev brief).** Pass the architect's full response and the plan file path. Team-lead reads `plans/CCR-NNN-<slug>.md` in full and returns `DISPATCH: <python-developer|web-developer> CCR-NNN` with `## Developer scope (CCR-NNN)`, `## Plan reference (CCR-NNN)`, and `## Reviewer focus (CCR-NNN)` sections.
9. **Dispatch the developer** that team-lead named. Pass the `## Developer scope (CCR-NNN)` section verbatim. If team-lead emitted a `## Plan reference (CCR-NNN)` block (architect path), include that too — the dev should open and follow `plans/CCR-NNN-<slug>.md`. Developer writes code + tests, runs self-checks, returns a structured implementation summary ending in `## BRIEF update note (CCR-NNN)` and the verdict line `READY FOR REVIEW: CCR-NNN`.
10. **Dispatch `team-lead` (Mode 1C — BRIEF/CONTEXT refresh, pre-review).** Pass the developer's *full* implementation summary, **including** the `## BRIEF update note (CCR-NNN)` section — verbatim, do not trim. Pass the original `## Reviewer focus (CCR-NNN)` block from team-lead Mode 1A or 1B so Mode 1C can reproduce it for the reviewer. Team-lead refreshes `docs/<feature>/BRIEF.md` and `docs/<feature>/CONTEXT.md` immediately, appends a Review log line, and returns one of:
    - `DISPATCH: reviewer CCR-NNN` with a reproduced `## Reviewer focus (CCR-NNN)` block → go to step 11.
    - `DISPATCH: <python-developer|web-developer> CCR-NNN` with a `## Fix scope (CCR-NNN)` section asking only for the missing BRIEF update note (dev's report was incomplete — the dev re-emits the report with the section added; no code change). Go back to step 9 with the fresh dev dispatch, passing the fix scope verbatim.
11. **Dispatch `reviewer` (solo).** Pass team-lead's reproduced `## Reviewer focus (CCR-NNN)` section from Mode 1C + a one-paragraph quote of the developer's "Files created or modified" list. If a plan file exists, point the reviewer at it so deviations get caught. The reviewer does code review + security checks + drift-check against the now-current `BRIEF.md` (Mode 1C just wrote it) + coverage audit, then runs the ticket's acceptance commands + targeted `pytest` + `ruff` + `mypy` at the end of its pass, and returns `REVIEW PASS: CCR-NNN` or `REVIEW FAIL: CCR-NNN — <summary>`. The reviewer does NOT audit whether the dev wrote the BRIEF section — Mode 1C already gated on that.
12. **Dispatch `team-lead` (Mode 2 — verdict).** Pass the reviewer's full response. (The dev report is also fine to include for context, but BRIEF/CONTEXT are already on disk.) Team-lead either:
    - Returns `APPROVED: CCR-NNN` (or `FEATURE COMPLETE: <feature>` for the feature's last ticket) — team-lead has ticked acceptance boxes, moved the ticket entry from `BACKLOG.md` to `DONE.md`, and (if applicable) flipped BRIEF `State: COMPLETE`. BRIEF and CONTEXT content was already written in Mode 1C. Proceed to step 13.
    - Returns `DISPATCH: <agent> CCR-NNN` with a `## Fix scope (CCR-NNN)` section in the body — ticket stays `[in-progress]` in `BACKLOG.md`. Go back to step 9 with a **fresh developer dispatch**, passing the fix scope verbatim. The dev session is new every time; the fix scope must be self-contained. (On the next pass, Mode 1C overwrites BRIEF/CONTEXT from the corrected dev report.)
13. **On APPROVED — pause for user approval.** This is a hard gate. **Do not** stage, commit, push, or run `gh pr create` until the user explicitly approves. End the turn by:
    ...
    - Stopping prompt execution. Wait for the user's reply. **Do not** continue to step 14 in the same turn.
14. **On user approval — create the PR.** Only after the user says go (e.g. "yes", "ship it", "approved"). Per `WORKFLOW.md §Integration`:
    - Stage the ticket's files plus the team-lead's `BACKLOG.md` / `DONE.md` / `CONTEXT.md` / `BRIEF.md` updates and the architect's `plans/CCR-NNN-<slug>.md` if it was created. Both `BACKLOG.md` (the cut) and `DONE.md` (the paste) are part of the same commit.
