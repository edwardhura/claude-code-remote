...
- **`bot/handlers/ask_user_question.py::_ID8_RE` regex differs from `bot/handlers/session.py::_HEX8_RE`.** Architect plan in `.claude/plans/CCR-026-ask-user-question.md` already considered and explicitly rejected lifting these to a shared module — the two regexes guard different identifier spaces. Documented in code comment at `ask_user_question.py:49-56`. Leave.
