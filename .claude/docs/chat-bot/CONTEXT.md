...
- `src/ccr/bot/formatting.py` — `chunk_text` (splits at paragraph/sentence boundaries, hard-cuts at 3500 chars) and `event_to_messages(event: ClaudeEvent) -> list[str]` (maps all ClaudeEvent variants to Telegram HTML-safe strings; unknown/permission variants return `[]`); all user-controlled strings escaped via `html.escape()`
- `src/ccr/bot/handlers/session.py` — `/new`, `/stop`, `/clear`, `/who` command handlers plus `handle_text` plain-text handler; injected `SessionManager` via aiogram workflow data; handles `SessionError` and `NoActiveSessionError` cleanly
- `src/ccr/server.py` — `async serve(settings)` instantiates `EventBus`, `SessionManager`, and a dedicated broadcast `Bot`; runs bot polling task and `_broadcast_loop` concurrently via `asyncio.gather`; `_ChatSender` implements per-chat `asyncio.Queue` + sender task for head-of-line-blocking isolation; `TelegramAPIError` swallowed and logged per sender
