...
import json
import uuid  # noqa: TC003 — resolved at runtime by pydantic for McpPermissionRequest
...
# --- synthetic bus envelopes ------------------------------------------------ #


class McpPermissionRequest(BaseModel):
    """Synthetic bus envelope generated by :class:`ccr.claude.mcp.McpPermissionServer`.

    NOT a member of :data:`ClaudeEvent` — it never appears as a JSONL line; the
    bus is the only place it lives. Telegram broadcast and SSE consumers branch
    on ``isinstance(payload['event'], McpPermissionRequest)``.
    """
