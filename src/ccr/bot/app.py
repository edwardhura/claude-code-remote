...
callback-query update streams, stashes the ``db_factory`` in workflow data
so handlers can open their own short-lived DB sessions, and includes the
pairing router. ``run_polling`` instantiates the :class:`Bot` and enters
``dp.start_polling``; the bot token is read from settings and never logged.
"""
...
async def run_polling(
    ...
    bot, dp = build_dispatcher(settings, db_factory)
