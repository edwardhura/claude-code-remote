...
## CCR-024: Remove dead permission-gating code (CCR-009 cleanup) [todo]
...
Acceptance:
  - [ ] No symbol named `PermissionRequest` survives anywhere in `src/ccr/` (verify via `grep -r "PermissionRequest" src/`).
  - [ ] No symbol named `send_permission_response` survives in `src/ccr/` (verify via `grep -r "send_permission_response" src/`).
  - [ ] No `TODO(CCR-019)` comment survives in `src/ccr/claude/process.py` (verify via `grep -n "TODO(CCR-019)" src/ccr/claude/process.py`).
