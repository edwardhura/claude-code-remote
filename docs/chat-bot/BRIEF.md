...
- `session.py::cmd_new` reply — `"New session started (pid {pid})."` (was `"Session {id8} started (pid {pid})."` before CCR-045).
- `session.py::cmd_continue` reply (happy path) — `"Session resumed (pid {pid})."` (was `"Session {id8} resumed (pid {pid})."` before CCR-045).
- `session.py::cmd_clear` reply (post-divider, post-new) — `"New session started (pid {pid})."` (was `"Session {id8} started (pid {pid})."` before CCR-045).
...
- Last updated: CCR-045 (2026-05-08)
