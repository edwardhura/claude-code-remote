...
- `/new` broadcasts a divider (`"— — — new session — — —"`) to all paired chats *only* when `prior_status != IDLE` — idle starts stay quiet. (CCR-047 collapsed `/clear` into `/new`; the divider behaviour previously owned by `/clear` now belongs to `/new`.)
...
- User-facing chat replies in `src/ccr/bot/` MUST NOT render the local `Session.id` UUID (or any prefix of it) as a user-visible handle. Pid is the user-visible handle for the active subprocess; `claude_session_id[:8]` is the user-visible handle for historical sessions in `/sessions` / `/continue` / `/rename`. Callback-data payloads (`perm:`, `auq:`, `cfg:`) keep the local UUID — they are server-controlled and not user-typed; this is intentional and exempt.
- `/stop` is intentionally no-arg under the single-session invariant. Adding a `<claude-id-prefix>` argument is rejected as a no-op: there is at most one active session globally, so a prefix arg can only either target the active session (redundant with the no-arg form) or target a non-active row (which is already stopped, so the reply is always "already stopped" noise). After CCR-044/CCR-045 there is no ID in the user-visible `/new` or `/stop` reply that a user would naturally copy-paste as a stop argument; pid is the only user-visible active-session handle and it is not a persistent address.
- `cmd_stop`'s "is there a subprocess to stop" discriminator MUST use `info().session_id` (mirrors `_proc is None`), NOT `status()`. After CCR-044, `status() == IDLE` is true both when no subprocess is held and during the held-but-pre-`SystemInit` `[idle]` window; using `status()` as the guard would incorrectly skip `stop()` in the second case. The CCR-046 fix codifies this — see `cmd_stop` docstring.
- `/answer <id8> <text>` is the ONLY bot command whose `<id8>`-shaped argument is not a `claude_session_id[:8]` reference. The `/answer` `<id8>` is an AskUserQuestion `tool_use_id` prefix matched by `ask_user_question.py::_ID8_RE = ^[0-9a-zA-Z_-]{8}$` (broader than the hex-only `_HEX8_RE` used by `/continue` / `/rename`). This is intentional namespace divergence — `tool_use_id` real-world prefixes contain `_` (e.g. `toolu_01`) and would be rejected by a hex-only regex. Future work that adds another id-prefix command must explicitly state which namespace it targets.
...
- `session.py::cmd_new` reply — `"New session started (pid {pid})."` (was `"Session {id8} started (pid {pid})."` before CCR-045). CCR-047: when invoked over a running session, `cmd_new` now performs the explicit `manager.stop()` + paired-chat divider broadcast (gated on `prior_status != IDLE`) before delegating to `manager.new_session(prompt=None, ...)`. The divider broadcast and the new-session start are sequenced (stop → divider → new) so the divider lands in chat order before the new session's first events.
- `session.py::cmd_continue` reply (happy path) — `"Session resumed (pid {pid})."` (was `"Session {id8} resumed (pid {pid})."` before CCR-045).
- removed: `session.py::cmd_clear` (CCR-047 collapsed into `cmd_new`; behaviourally identical — `/new` now owns the divider broadcast that `/clear` previously owned).
- `session.py::cmd_pid` reply (active session) — `"Session running · pid {pid} · uptime {uptime}"` (was `"Session {id8} · pid {pid} · running {uptime}"` before CCR-045).
- removed: `session.py::_short_id` private helper (dead after CCR-045 rewrote all four user-facing call sites; function gone, no callers remain).
- `session.py::cmd_stop` — guard reworked to use `inf.get("session_id") is None` rather than `status() == IDLE` so the CCR-044 `[idle]` window (subprocess held, pre-`SystemInit`) is correctly classified as stoppable. Reply strings unchanged: `"No active session."` when no subprocess is held, `"Session stopped."` after a successful stop. Still no-arg by deliberate design (CCR-046 audit decision — see Key invariants).
...
- `passthrough.py` — three-branch dispatch: `WHITELIST = {"model", "compact"}` forwarded via `manager.send_slash`; `BLOCKED_INTERACTIVE = {"mcp", "init"}` returns canned redirect; unknown commands return usage hint. Dedicated branches for `/agents` (Running + Project agents + Built-in agents), `/skills`, `/cost`, `/usage`. `/cost` rich reply no longer includes a `<b>Session:</b> {id8}` line (removed in CCR-045); reply starts at `<b>Turns:</b>`, six lines (seven with cost), HTML-formatted.
  ...
  - `passthrough.py::_render_cost_reply(usage: SessionUsage) -> str` — no longer accepts a `session_id_hex` argument (removed in CCR-045); no longer emits the `<b>Session:</b> {id8}` line. `_reply_cost` passes only `usage`.
  - removed: `passthrough.py::_SESSION_ID_HEX_PREFIX_LEN` private constant (dead after CCR-045 `_render_cost_reply` lost its session-id arg).
...
- **`/new` divider** is gated on `prior_status != IDLE` and the read must happen before `manager.stop()` (status flips after stop). CCR-047: this divider behaviour was previously owned by `/clear`; `/new` inherited it when `/clear` was collapsed.
...
- **/cost deliberately drops the session-id slot rather than substituting `claude_session_id[:8]`.** Rationale: `/cost` only fires when a subprocess is held, so the implicit "this session" is unambiguous; the manager does not currently expose `claude_session_id` as a property and a DB lookup just to render a label was rejected as overkill. If a future ticket needs the slot back, the path is "add `current_claude_session_id` property to `SessionManager`, thread it through `_reply_cost` → `_render_cost_reply`".
- **`cmd_continue` no longer captures the return value of `session_manager.continue_session`** — the manager still returns the local UUID, but the bot layer no longer needs it for any reply.
- **`cmd_pid` reply format is `"Session running · pid {pid} · uptime {uptime}"`** (kept uptime). The team-lead's scope showed `"Session running (pid {pid})."` as the "or equivalent" target; uptime was kept because it's useful and independent of the UUID concern.
- **Grep canary for local-UUID surfaces in bot handlers:** `grep -rn "session\.id\|Session\.id\|_short_id\|\.hex\[:8\]" src/ccr/bot/` should return only pedagogical comments and one `WHERE Session.id == ...` SQL clause. No user-facing reply strings. Future tickets touching the bot layer can run this grep to confirm no regression.
- **CCR-046 per-command audit:**

  | Command | Takes session-reference arg? | Namespace of the arg (if any) | Status |
  |---|---|---|---|
  | `/start` | No | — | no change needed |
  | `/new` | No | — | no change needed (CCR-047: now owns the divider broadcast that `/clear` had) |
  | `/stop` | No | — | intentionally no-arg (single-session invariant); rekey is no-op (CCR-046) |
  | ~~`/clear`~~ | — | — | **REMOVED in CCR-047** — collapsed into `/new` (was a duplicate of `/new` + divider) |
  | `/who` | No | — | no change needed |
  | `/pid` | No | — | no change needed |
  | `/sessions` | No | — | listing rekeyed to `claude_session_id[:8]` in CCR-042 |
  | `/continue <prefix>` | Yes | `claude_session_id[:8]` | rekeyed in CCR-042 (done) |
  | `/rename <prefix> <name>` / `/rename current <name>` | Yes | `claude_session_id[:8]` / `current` keyword | rekeyed in CCR-043 (done) |
  | `/answer <id8> <text>` | Yes | `tool_use_id` (AskUserQuestion) | **intentionally different namespace** — NOT a session reference |
  | `/agents` | No | — | no change needed |
  | `/skills` | No | — | no change needed |
  | `/cost` | No | — | no change needed |
  | `/usage` | No | — | no change needed |
  | `/config` | No (sub-command `tz <iana>` only) | — | no change needed |
  | `/model` | No | — | no change needed (passthrough to `claude` CLI) |
  | `/compact` | No | — | no change needed (passthrough to `claude` CLI) |
  | `/mcp` | No (canned redirect) | — | no change needed |
  | `/init` | No (canned redirect) | — | no change needed |

- **The CCR-046 audit confirms the chat-bot's "no local-UUID surface in user-facing replies" invariant** (introduced in CCR-045 and tightened here). The grep canary `grep -rn "session\.id\|Session\.id\|_short_id\|\.hex\[:8\]" src/ccr/bot/` returns only docstring references and one SQL-clause hit (`Session.id == local_session_id` in `_handle_rename_current`); no user-facing reply string in any handler renders a local-UUID prefix.
- **The `cmd_stop` discriminator change is observably distinct from `cmd_pid`'s deliberately different choice.** `cmd_pid` keeps the `status() == IDLE OR session_id is None` guard because its semantic is "Claude is not yet usable from the user's perspective" — the `[idle]` window correctly returns "No active session." for `/pid`. `cmd_stop`'s semantic is "release the held subprocess" — the `[idle]` window must trigger an actual stop. Future tickets touching either handler must respect this asymmetry: `/pid` and `/stop` are NOT meant to share a guard helper.
