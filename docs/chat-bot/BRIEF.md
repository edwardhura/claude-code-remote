...
- `/sessions` rows render the first 8 hex characters of `Session.claude_session_id` (HTML-escaped) when set; rows with `claude_session_id IS NULL` render the literal `<code>--------</code>` marker (8 ASCII dashes, same column width as a hex prefix) — never a local-UUID prefix. The full Claude id is never displayed in the listing.
...
- `session.py::_NULL_CLAUDE_SESSION_ID_MARKER = "--------"` — module-level constant: the 8-character ASCII-dash marker rendered for `/sessions` rows whose `claude_session_id IS NULL`. Same column width as the 8-hex prefix; can never collide with a real `claude_session_id[:8]` because hex digits are `[0-9a-f]` and `-` is not a hex digit.
...
- **Two prefix-match conventions coexist in `src/ccr/bot/handlers/session.py`.** `cmd_continue` (and the matching `_db_lookup_resumable_claude_session_id` in the manager) compares `<prefix>` against `claude_session_id[:8]` per CCR-042. `cmd_rename`'s `_parse_rename_args` still compares against `str(row.id)[:8]` (the local UUID prefix); aligning the two is CCR-043's job. Mixing them in a future ticket would silently fail-or-mismatch.
