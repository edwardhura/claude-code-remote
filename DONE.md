...
## CCR-038: `/answer` HTML parse-error fix + bot-wide static-string audit [done]
Phase: n/a (bugfix — chat-bot UX, follow-up to CCR-026)
Feature: chat-bot
Files:
...
Acceptance:
  - [x] `/answer` with no args replies with the HTML-escaped usage hint (`Usage: /answer &lt;8-char-id&gt; &lt;text&gt;` rendered as `Usage: /answer <8-char-id> <text>` in Telegram) and does NOT raise `TelegramBadRequest`.
  - [x] Reproducer no longer triggers: typing `/answer` with no args produces a clean reply, not a stack trace into `cmd_answer`.
  - [x] The static-string audit found no further raw-`<...>` cases (or fixed all that it found — list them in the developer's work summary).
  - [x] The defensive `try/except TelegramBadRequest` is scoped to the static-string send only; no broader exception catch.
  - [x] `pytest tests/test_bot_ask_user_question.py` passes.
  - [x] `pytest --cov=ccr --cov-fail-under=80` passes.
Depends on: CCR-026
Notes:
...
## CCR-040: `/agents` reply mirrors Claude Code CLI library view [done]
...
Acceptance:
  - [x] Each project-agent line renders as `<name> · <model>`; missing `model:` frontmatter renders `inherit`.
  - [x] A `Built-in agents` section is present, listing every entry in the constant `BUILTIN_AGENTS` map as `<name> · <model>`.
  - [x] `BUILTIN_AGENTS` has a comment noting the Claude Code version it was sourced from (so future maintainers know to revisit).
  - [x] Final section order in the `/agents` reply is Running → Project agents → Built-in agents.
  - [x] HTML escaping is preserved for agent names containing `<`, `>`, `&` (regression vs. CCR-022).
  - [x] `pytest tests/test_bot_passthrough.py` passes.
  - [x] `pytest --cov=ccr --cov-fail-under=80` passes.
Depends on: CCR-022
Notes:
...
### Review log
  - 2026-05-06 main: branch ccr-040-agents-model-info created, dispatching team-lead
  - 2026-05-06 team-lead: scope brief issued (no architect), dispatching python-developer
  - 2026-05-06 team-lead: BRIEF/CONTEXT refreshed, dispatching reviewer
---

## CCR-041: `/continue` UNIQUE-constraint regression — drop unique flag on `claude_session_id`, programmatic duplicate guard [done]
...
Acceptance:
  - [x] Fresh session followed by `/continue` no-prefix produces no `claude_session_id_update_failed` log entry; the resumed row's `claude_session_id` equals the prior row's value.
  - [x] `/continue <8-hex-prefix>` against the resumed row succeeds (does not raise `NoPriorSessionError`).
  - [x] `import_claude_session` still raises `DuplicateClaudeSessionError` when called twice for the same `claude_session_id` (programmatic guard intact).
  - [x] Multiple rows can share the same `claude_session_id` without `IntegrityError` (regression test in `tests/test_session_manager.py`).
  - [x] `pytest --cov=ccr --cov-fail-under=80` passes.
  - [x] `ruff check src tests` passes.
...
### Review log
  ...
  - 2026-05-06 main: branch ccr-041-continue-unique-fix created, dispatching team-lead
  - 2026-05-06 team-lead: scope brief issued (no architect), dispatching python-developer
  - 2026-05-06 team-lead: BRIEF/CONTEXT refreshed, dispatching reviewer
  - 2026-05-06 team-lead: approved
  - 2026-05-06 team-lead: approved
...
## CCR-044: Persist `[idle]` status; transition to `[running]` on first `claude_session_id`; filter idle rows from `/sessions` and prefix lookups [done]
...
Acceptance:
  - [x] A fresh `new_session(...)` call inserts a `Session` row with `status = "idle"` and `claude_session_id IS NULL`.
  - [x] On the first `SystemInit` event arriving for that session, the row's `status` transitions to `"running"` and `claude_session_id` is populated in the same UPDATE (single round-trip).
  - [x] A second / later `SystemInit` event for the same session does NOT re-flip `status` (idempotent, single-shot semantics preserved per CCR-036's `_claude_session_id_persisted` guard).
  - [x] `/sessions` does not list rows where `status == "idle"`.
  - [x] `_db_lookup_resumable_claude_session_id` skips idle rows from prefix matching: a prefix that matches only an idle row raises `SessionNotFoundError` (consistent with the NULL-`claude_session_id` skip behaviour from CCR-042).
  - [x] An additive Alembic migration documenting `idle` as a valid `Session.status` value is added (forward-only; pre-existing NULL-`claude_session_id` rows are NOT backfilled to `idle` and keep their existing terminal status).
  - [x] `pytest --cov=ccr --cov-fail-under=80` passes.
  - [x] `ruff check src tests` passes.
  - [x] `ruff format --check` passes.
  - [x] `mypy src` passes.
...
### Review log
  ...
  - 2026-05-08 main: branch ccr-044-persist-idle-status created, dispatching team-lead
  - 2026-05-08 team-lead: dispatching architect — state machine invariant flip + single-vs-two UPDATE decision + 4-ticket slice anchor
  - 2026-05-08 team-lead: plan reviewed (plans/CCR-044-persist-idle-status.md), dispatching python-developer
...
## CCR-043: `/rename` rekeyed to `claude_session_id[:8]`; add `/rename current <name>` for active session [done]
...
Acceptance:
  - [x] `/rename <claude-id-prefix> <new-name>` renames every row whose `claude_session_id[:8]` equals the prefix; the local-UUID prefix lookup is removed.
  - [x] `/rename <prefix> <new-name>` where the prefix only matches NULL-`claude_session_id` rows replies with the documented unknown-prefix error and does not mutate.
  - [x] `/rename current <new-name>` renames the active session's chain when it has a non-NULL `claude_session_id`; standard success reply.
  - [x] `/rename current <new-name>` with no active session replies `"No active session to rename."` (verbatim) and does not mutate.
  - [x] `/rename current <new-name>` with an active session whose `claude_session_id IS NULL` (status `[idle]` from CCR-044) replies with a distinct stable error (different wording from the no-active-session reply) and does not mutate.
  - [x] `/rename` with no args (or only whitespace) replies with the usage hint and does not mutate; the usage hint documents both `<prefix>` and `current` forms with HTML-safe `&lt;`/`&gt;`.
  - [x] `pytest --cov=ccr --cov-fail-under=80` passes.
  - [x] `ruff check src tests` passes.
  - [x] `ruff format --check` passes.
  - [x] `mypy src` passes.
